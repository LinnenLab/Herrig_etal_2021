##modified from rnaseq_plot_funcs.R script found in the Trinity Differential Expression Analysis package
...
plot_Volcano = function(logFoldChange, FDR, xlab="logFC", ylab="-1*log10(FDR)", title="Volcano plot", pch=20) {
   ...
   plot(logFoldChange, -1*log10(FDR), col=ifelse(FDR<=0.05, "red", "black"), xlab=xlab, ylab=ylab, ylim=c(0,200), xlim=c(-30,30), main=title, pch=pch);
